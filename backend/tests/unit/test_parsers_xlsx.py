"""XLSX parser: per-row segmentation keyed by (sheet, row_index)."""
...
def test_parse_xlsx_emits_one_segment_per_row_per_sheet():
    """parse() emits one xlsx-typed segment per data row per sheet."""
...
def test_excerpt_returns_row_text():
    """excerpt() returns the rendered row at the requested (sheet, row_index)."""
    pf = parse(file_id="f1", file_name="leads.xlsx", path=FIXTURE)
