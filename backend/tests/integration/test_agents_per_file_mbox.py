"""Per-file Mbox agent against real Ollama + parser.
...
from app.agents.per_file import mbox as mbox_agent
from app.agents.per_file._tools.cite_locator import cite_locator
from app.config import get_settings
from app.llm import get_provider
from app.parsers import mbox as mbox_parser
...
def test_per_file_mbox_emits_valid_file_summary():
    ...
    parsed = mbox_parser.parse(file_id="f1", file_name="inbox.mbox", path=fixture)
    get_provider.cache_clear()
    fs = mbox_agent.run(provider=get_provider(), parsed=parsed)
