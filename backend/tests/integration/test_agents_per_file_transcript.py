"""Per-file VTT transcript agent against real Ollama + parser.
...
from app.agents.per_file import transcript as transcript_agent
from app.agents.per_file._tools.cite_locator import cite_locator
from app.config import get_settings
from app.llm import get_provider
from app.parsers import vtt as vtt_parser
...
def test_per_file_transcript_emits_valid_file_summary():
    ...
    parsed = vtt_parser.parse(file_id="f1", file_name="call.vtt", path=fixture)
    get_provider.cache_clear()
    fs = transcript_agent.run(provider=get_provider(), parsed=parsed)
