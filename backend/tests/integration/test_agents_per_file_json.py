"""Per-file JSON agent against real Ollama + parser.
...
from app.agents.per_file import json as json_agent
from app.agents.per_file._tools.cite_locator import cite_locator
from app.config import get_settings
from app.llm import get_provider
from app.parsers import json as json_parser
...
def test_per_file_json_emits_valid_file_summary():
    ...
    parsed = json_parser.parse(file_id="f1", file_name="crm.json", path=fixture)
    get_provider.cache_clear()
    fs = json_agent.run(provider=get_provider(), parsed=parsed)
